...
## Connecting the headset using Oculus Link (optional)
This is only required to use the Oculus Headsets with the cable connected to the PC while running the EESP editor. 
This is not suitable for trainings, but is useful so you more easily if objects in your training are correctly placed.
...
6. Press the "VR Preview" in the bottom right corner.
